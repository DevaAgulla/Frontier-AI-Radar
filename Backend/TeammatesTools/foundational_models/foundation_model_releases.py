...
Configuration (URLs, provider maps, timeouts, …) lives in config.py.
...
def fetch_foundation_model_releases(
    ...
    ----------
    urls:
        List of feed / API URLs to query. Defaults to SOURCE_URLS in config.py
        when called from the CLI, but callers can pass any list.
    current_date:
...
if __name__ == "__main__":
    ...
    # Accept an optional date argument: YYYY-MM-DD (defaults to today)
    target = date.fromisoformat(sys.argv[1]) if len(sys.argv) > 1 else date.today()

    # SOURCE_URLS is imported from config.py
    results = fetch_foundation_model_releases(SOURCE_URLS, target)
